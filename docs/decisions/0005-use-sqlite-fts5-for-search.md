...
- The unicode61 tokenizer splits on non-letter, non-digit characters only. An identifier like `AuthorizationService` is indexed as a single token `authorizationservice`; an identifier like `with_underscores` would normally split into two tokens, but the `tokenchars '$_'` configuration keeps it as `with_underscores`. Prefix queries against the start of an identifier work natively: `Auth` matches `AuthorizationService` because the token `authorizationservice` has the prefix `auth`. Substring queries that target the middle of an identifier (`Service` against `AuthorizationService`) do not match, because the token does not have that prefix. CamelCase-fuzzy matching (`AuthSvc` → `AuthorizationService`) is a separate and harder problem; see "Alternatives Considered" for why deferring it is the right call now.
